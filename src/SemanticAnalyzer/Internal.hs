...
instance Show Definition where
  show (Definition name expr _) = show name ++ " := " ++ show expr
...
mkProgram (ProgInfDeps uprog (DependencyList dList)) = do
  ...
  where
    ...
    helperTree uprog' a (Right prevDeps) =
      (: prevDeps)
        <$> ( (\e -> Definition a e (udefPos udef))
                <$> mkInfExprTree prevDeps udef
            )
      where
        udef = lookupUDefUnsafe uprog' a
    helperCycle ::
      UninfProg ->
      [L.VarIdent] ->
      Either TypeError [Definition] ->
    ...
    helperCycle uprog' as (Right prevDeps) =
      ...
      where
        udefs = lookupUDefUnsafe uprog' <$> as
    ...
    lookupUDefUnsafe (UninfProg udefs) sname =
      case find (\(UninfDefinition name _ _ _) -> name == sname) udefs of
        ...
        Just a -> a
    checkMainFn :: [Definition] -> Either TypeError [Definition]
...
lookupRefType parts refName =
  ( find (\(Definition name _ _) -> name == refName) parts
      >>= ( \case
              (Definition _ (WishTyExpr _ wtyp) _) -> Just wtyp
              (Definition _ (InfTyExpr infExpr) _) -> Just (ieType infExpr)
          )
  )
    <|> (\(_, a, _) -> a)
