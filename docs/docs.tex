...
During this process all type information is discarded, as it is no longer
needed as well as lambda expressions are renamed to closures, and they acquire
a depth field used to determine the number of captured values they hold.

% todo: add info about gc_data
\subsubsection{Garbage collection}

In the C source code that the Code Generator generates, there are different
...
it and the objects it refers to and marks them with a separate flag. Once the
whole linked list is walked, the garbage collector walks the list again,
discarding the objects that are not marked.

\subsubsection{Closures}

Closures are objects in the generated C source code that represent a function.
All closures have a field named \verb$clfunc$, that holds a function pointer.
All \verb$clfunc$ function pointers take two pointers as arguments. Their first
argument should always point to the closure object and their second argument
...
\verb$gc_type$ is a structure that has the same memory
layout for its garbage collection information as any other object, however it
does not have any other fields specified. \verb$gc_type$ is used as a generic
type in the garbage collector, as any object can be cast to it without losing
