...
Throughout this document, new concepts will be described the first time they are
used. To introduce new names for specific concepts, the new names will be
written with an \emph{italic font} for emphasis.
...
formal system of lambda calculus, and the LC language. There have been some
...
type-safety. Type inference makes the code more concise by removing the need
to specify types. Type inference can also make the code less understandable, as
it can remove the specification of important properties of functions. For this
reason it was decided that writers of the LC language will be allowed to specify
types if it seems important. Specifying types also has the benefit of
constraining the domains of functions, as the type inference system might infer
a function domain that is too general.
...
the extension \texttt{.lc}. To simply compile an input file, one would run the
following:
\begin{lstlisting}
$ lcc hello.lc
\end{lstlisting}
...
types support parametric polymorphism. In this section, the key details of
...
one expression to another in the LC language, one would write the function to be
...
as local variables; therefore in name-collisions, the most local variable will
take precedence.
...
\end{tikzpicture}
\end{center}
While for an input graph with a cycle, the algorithm would return
\begin{center}
\begin{tikzpicture}[->]
    \node (a) at (1,3) {A};
    \node (b) at (3,3) {B};
...
\begin{enumerate}
    \item Build an $n \times n$ binary $G$ matrix, where $n$ is the number of
        ...
        contains ones.) The matrix of the above graph would be the
        following:
\begin{center}
\begin{tikzpicture}[->]
    \node (a) at (1,3) {A};
...
the nodes that depended on either $A$ or $B$ will need to depend on $C$.
...
\item Once all the cycles are merged, the ordering of the dependency list can be
    ...
    dependency matrix for this purpose, as leaves in the graph will have exactly
    one $1$s in their row. Removing a leaf is simulated by removing its
