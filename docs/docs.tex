...
\end{lstlisting}
Each cell defines one state transition, and to encode more than one cells, one
would need to separate them with two ones.

A cell can be encoded in the following way:
\begin{lstlisting}
<state>1<read>1<new state>1<write>1<direction>
\end{lstlisting}
\lstset{frame=lines}
\lstset{numbers=left}
