...
is a datatype that can be constructed using a list and a \emph{dependency
function}
...
\end{center}
    ...
    The composition matrix can be computed using several methods. For example,
    one intuitive way is to use the $(n-1)$th power of the matrix, as the
    longest path can only contain $n$ nodes, and $(n-1)$ edges: $G^{(n-1)} =
    G'$.

    However, there is a more optimal way of finding the composition matrix. By
...
investigated, and a new code change is created which is tested in the same way.
