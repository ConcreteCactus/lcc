...
the standard library. It binds to the \verb$getchar$ function in the C language.
Similarly to the \verb$getchar$ function in the C language, \verb$getchar$ in
the LC language return an integer that, along with the \verb$eof$ function, can
be used to check whether the end of the file is reached.
\begin{lstlisting}
getchar : I32
eof     : I32
\end{lstlisting}
% note on the let function

\paragraph{Constructors and basic operations on primitives} 
